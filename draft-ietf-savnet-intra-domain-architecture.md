...
One possible approach is to use routing-derived and operator-provisioned source prefixes. For each entity, the SAV Agent can derive a set of source prefixes by aggregating routing information associated with the interfaces connected to that entity. By using the entity-interface associations and the prefix reachability information associated with each interface, the SAV Agent can obtain a more complete set of routing-derived source prefixes than would be available from a single interface. These prefixes can be updated automatically as the corresponding routing information changes.

The AS operator can also provision source prefixes that an entity is authorized to use for sourcing traffic. Such prefixes may include:
