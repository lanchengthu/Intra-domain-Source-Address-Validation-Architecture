...
A SAV Agent may use routing information, SAV-specific information, or both for SAV rule generation, depending on the mechanism and the information available. A specific mechanism needs to define how the available information is used to determine the source address space permitted on each relevant external interface and how incomplete, stale, or conflicting information is handled.
